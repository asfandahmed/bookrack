	...
	public function find($keywords,$filters="")
	{
		...
		elseif($filters=='authors'){
			...
			$query="MATCH (n:Book)
				...
				OPTIONAL MATCH (n)<-[:WROTE]-(o:Author)
				RETURN ID(n), n.title AS name, n.image as image, labels(n) as type, COUNT(DISTINCT m) as owners, 
				o.name as author, '' as books, '' as followers, '' as relation
				...
				COUNT(DISTINCT b) as books, COUNT(DISTINCT m) as followers, r.date_time as relation, '' as owners, '' as author

				UNION ALL
				
				MATCH (n:Publisher)
				WHERE n.company =~ '(?i)".$keywords.".*'
				RETURN ID(n), n.company AS name, n.image as image, labels(n) as type, '' as books, '' as followers, '' as relation, '' as owners, '' as author
				
				UNION ALL 
				
				MATCH (n:Author)
				WHERE n.name =~ '(?i)".$keywords.".*' 
				RETURN ID(n), n.name AS name, n.image as image, labels(n) as type, '' as books, '' as followers, '' as relation, '' as owners, '' as author";
		}
